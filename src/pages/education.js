    ...
    return (
      <>
        <div className="container">
          ...
          <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-3 ml-4">
            {this.state.education.map((edu) => (
               <div className="col mb-4" key={edu.id}>
              <div className="card ml-4 d-flex align-items-center">
                <img src={edu.image} className="card-img-top" alt={edu.degree} />
                <div className="card-body">
                  <h3 className="card-title">{edu.degree}</h3>
                  <p className="card-text">{edu.university}</p>
                  <p className="card-text">{edu.year}</p>
                  <p className="card-text">{edu.percentage}</p>
                  </div>
                </div>
              </div>
            ))}
